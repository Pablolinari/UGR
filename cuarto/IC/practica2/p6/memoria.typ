...
== Especificación de los módulos desarrollados
...
+ *Módulo 2 — Adquisición de información del usuario* (diálogo justificado).
...
- *Conocimiento que deduce (alto nivel):* tipo de plato (si falta), ingrediente relevante, y las propiedades de picante, gluten, lactosa, no-vegetariana/no-vegana, vegetariana/vegana, sin gluten/sin lactosa, nivel de calorías y nivel de proteínas.
...
- *Conocimiento que utiliza:* los hechos `(pregunta ...)` pendientes, las funciones de validación de dominio y `unico-campo-base` (comprueba si todas las recetas compatibles comparten el mismo valor en un campo).
- *Conocimiento que deduce (alto nivel):* el perfil de necesidades del usuario (sus respuestas) y la lista de preguntas omitidas por no aportar información.
...
- *Objetivo:* descartar (filtrado duro) las recetas que incumplen una restricción innegociable del usuario.
- *Conocimiento que utiliza:* las respuestas de tipo restricción y las propiedades deducidas en el Módulo 1.
- *Conocimiento que deduce (alto nivel):* el conjunto de recetas *candidatas* que cumplen todas las restricciones.
...
- *Hechos que deduce / modifica:* crea las `(RecetaCandidata ...)` iniciales y *retira* las que incumplen una restricción; avanza la `(fase ...)`.
...
- *Conocimiento que deduce (alto nivel):* la evidencia a favor/en contra de cada receta y su *factor de certeza combinado*; si el usuario no expresó ninguna preferencia, se aplica el supuesto por defecto de preferir recetas fáciles.
...
- *Conocimiento que utiliza:* las `RecetaCandidata` con su CF, las evidencias acumuladas, las preguntas omitidas y las restricciones del usuario; funciones `explicar-receta`, `etiqueta-confianza`, `prob-acierto-receta` (razonamiento probabilístico) y `unir-frases`.
- *Conocimiento que deduce (alto nivel):* la receta recomendada y su justificación en lenguaje natural; en caso de rechazo, la siguiente mejor; en caso de reajuste, relanza el razonamiento.
...
= Breve manual de uso del sistema

+ *Arranque de CLIPS.* Abrir CLIPS en el directorio de la práctica.
