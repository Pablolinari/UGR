...
; last_of_maxima es una función que calcula el last_of_maxima del conjunto difuso
;trapezoidal (a b c d) cortado a una altura alpha.
(deffunction last_of_maxima (?a ?b ?c ?d ?alpha)
(bind ?rv (- ?d (* ?alpha (- ?d ?c))))
?rv
)

 ;;;;;;;  Elegimos el método de defuzzificacion
 
 (deffacts Metodo_defuzzificacion
(metodo defuzzificacion COS) ;  center of sums 
;(metodo defuzzificacion  LOM) ;  last of maxima
 )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Obtenemos el valor deducido defuzzificando 
;;; 1) como la media ponderada (por los grados de cada consecuente)  de los centros de gravedad de los consecuentes inferidos en el COS
;;; 2) como el mayor valor que tenga el maximo grado de la salida difusa en el caso del LOM
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defrule etiqueta_maximo_grado_salida_difusa_primer_candidato
(modulo calculo_fuzzy)
(metodo defuzzificacion LOM) 
(fuzzy inferido ?v ?l ?g1)
(not (etiqueta_salida_maximo_grado ?v ? ?))
=>
(assert (etiqueta_salida_maximo_grado ?v ?l ?g1))
)

(defrule etiqueta_maximo_grado_salida_difusa_actualizar
(declare (salience 1))
(modulo calculo_fuzzy)
(metodo defuzzificacion LOM) 
(fuzzy inferido ?v ?l ?g2)
?f <- (etiqueta_salida_maximo_grado ?v ?l2 ?g1)
(test (> ?g2 ?g1))
=>
(assert (etiqueta_salida_maximo_grado ?v ?l ?g2))
(retract ?f)
)

(defrule etiqueta_maximo_grado_salida_difusa_actualizar2
(modulo calculo_fuzzy)
(metodo defuzzificacion LOM) 
(fuzzy inferido ?v ?l ?g2)
(etiqueta_salida_maximo_grado ?v ?l2 ?g2)
(test (neq ?l ?l2))
=>
(assert (etiqueta_salida_maximo_grado ?v ?l ?g2))
)

(defrule valor_maximo_salida_maximo_grado
(modulo calculo_fuzzy)
(metodo defuzzificacion LOM) 
(etiqueta_salida_maximo_grado ?v ?l ?g)
(cd ?v ?l ?a ?b ?c ?d)
=>
(assert (valor_maximo_grado ?v (last_of_maxima ?a ?b ?c ?d ?g)))
)

(defrule valor_maximo_grado_salida
(declare (salience 2))
(modulo calculo_fuzzy)
(metodo defuzzificacion LOM) 
?f <- (valor_maximo_grado ?v ?x1)
(valor_maximo_grado ?v ?x2)
(test (> ?x2 ?x1))
=>
(retract ?f)
)

(defrule respuesta_LOM
(declare (salience -1))
(modulo calculo_fuzzy)
(metodo defuzzificacion LOM) 
(valor_maximo_grado ?v ?x)
=>
(assert (fuzzy valor_inferido ?v ?x))
(printout t "Aplicando esta(s) regla(s) el valor de " ?v " es " ?x  crlf)
)
