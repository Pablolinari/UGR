...
func calcNormalesnuevas(verts: PackedVector3Array, tris: PackedInt32Array) -> PackedVector3Array:
	## Paso 1: comprobar precondiciones
	assert(verts.size() >= 3, "CalcNormales: la malla debe tener al menos 3 vértices")
	assert(tris.size() % 3 == 0, "CalcNormales: el número de enteros en 'tris' debe ser múltiplo de 3")
	...
	# Paso 2: inicializa normales a cero (más eficiente)
	var normales = PackedVector3Array()
	normales.resize(nv)
	for i in range(nv):
	...
	for it in range(nt):
		...
		var b = verts[i1]
		var c = verts[i2]
		var normalv = (c - a).cross(b - a).normalized()  # Nota: (b-a).cross(c-a) para CCW consistente
		normales[i0] += normalv
		normales[i1] += normalv
		normales[i2] += normalv
	...
	# Paso 4: normalizar normales
	for i in range(nv):
		if normales[i].length() > 0.001:  # Evita división por cero si suma ~0
			normales[i] = normales[i].normalized()
		else:
			normales[i] = Vector3.UP  # Default si algo sale mal (ajusta según tu orientación)
